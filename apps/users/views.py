...
def confirm(request, user_id, token):
    ...
    if not user.confirmationcode:
        return http.HttpResponseRedirect(reverse('cas_login'))
...
@login_required
def delete(request):
    ssouser = request.user.get_profile()
...
def edit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if ssouser.email != original_email:
                ...
                messages.info(request, _(('An email has been sent to {0} to '
                    ...
                    'current email address.')).format(ssouser.email))

                domain = settings.DOMAIN
                token, hash = EmailResetCode.create(ssouser.id, ssouser.email)
                url = "%s%s" % (settings.SITE_URL,
                                reverse('users.emailchange', args=[ssouser.id,
                                                                token, hash]))
                t = loader.get_template('users/email/emailchange.ltxt')
                c = {'domain': domain, 'url': url, }
                send_mail(_(("Please confirm your email address "
                             "change at %s") % domain),
                    t.render(Context(c)), None, [ssouser.email])

                # Reset the original email back.  We aren't changing their
                # address until they confirm the new one
                ssouser.email = original_email
            form.save()
        ...
        form = forms.UserEditForm(instance=ssouser)
...
def profile(request, user_id):
    ...
    user.sites = []
    return jingo.render(request, 'users/profile.html', {'user': user})
